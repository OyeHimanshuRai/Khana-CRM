...
|--------------------------------------------------------------------------
|
| Rendered by resources/views/components/icon.blade.php as <x-icon name="…" />.
|
| Inlined rather than pulled from an icon font or CDN so the panel keeps
| working offline and needs no build step. Everything is drawn on a 24x24
| grid in stroke style, with no fill.
|
| Lifted out of the component so screens that let an operator *choose* an
| icon - Services, for one - can enumerate and validate the same list rather
| than keeping a second copy of it in sync.
|
| The markup here is a fixed internal map, never user input, which is what
| makes the unescaped echo in the component safe.
|
*/
